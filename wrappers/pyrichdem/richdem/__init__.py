...
def DelineateCatchment(
  ...
  if method == "mflow":
    ...
    if x0 is not None and y0 is not None and x1 is not None and y1 is not None:
      _richdem.DC_line_mflow(x0,y0,x1,y1,demw,resultw)
    elif x0 is not None and y0 is not None:
      _richdem.DC_line_mflow(x0,y0,x0,y0,demw,resultw)
    elif mask is not None:
      maskw = mask.wrap()
      _richdem.DC_mask_mflow(maskw,demw,resultw)
  else:
    propsw = FlowProportions(dem,method).wrap()
    if x0 is not None and y0 is not None and x1 is not None and y1 is not None:
      _richdem.DC_line_props(x0,y0,x1,y1,propsw,resultw)
    elif x0 is not None and y0 is not None:
      _richdem.DC_line_props(x0,y0,x0,y0,propsw,resultw)
    elif mask is not None:
      maskw = mask.wrap()
      _richdem.DC_mask_props(maskw,propsw,resultw)
